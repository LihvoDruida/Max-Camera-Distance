...
L["PRESET_SECTION_DESC"] = "预设会把高级选择转换为实际距离。如果选择的不是“手动”，对应的滑块会自动锁定。"
...
L["MANUAL_SECTION_DESC"] = "只有当对应预设设置为“手动”时，手动控制才会可用。"
...
L["DEBUG_LOGGING_DESC"] = "这些选项控制哪些信息会输出到聊天框。即使关闭日志，上方的实时状态仍然始终可用。"
...
L["DELAY_HEADER_DESC"] = "战斗时拉远是立即生效的。返回常规距离会根据不同上下文延迟执行，以避免镜头闪烁。"
...
L["AFK_HIDE_UI_DESC"] = "为获得更具电影感的 AFK 模式，隐藏整个界面。按 ESC 可安全恢复，无需等待 AFK 标记重置。"
...
L["ZOOM_RESTORE_SETTING_DESC"] = "控制 Smart Zoom 如何恢复先前使用过的缩放值。永不 = 始终使用已配置的目标。自适应 = 仅在返回到刚刚离开的状态时恢复上一次缩放。始终 = 只要未超出该状态上限，就优先使用该状态最后保存的缩放值。"
